...
{
    private final static int[] POSSIBLE_VECTOR_COORDINATES={-9,-8,-7,-1,1,7,8,9};
    ...
    public List<Move> availableMoves(Board board) {
        ...
        for(final int currentPossibleMove: POSSIBLE_VECTOR_COORDINATES)
        {
            ...
            while(BoardUtils.isValidMoveCoordinate(candidateBoardCoordinate))
            {
                ...
                {   /*If we are in either the first column or last column, special exceptions apply for this specific vector*/
                    break;
                }
                final Tile possiblePiecePositionTile = board.getTile(candidateBoardCoordinate);
                if (!possiblePiecePositionTile.isTileOccupied())
                {
                    /*This queen is the second argument*/
                ...
                else
                {
                    ...
                    /*if is occupied by the opposite color from this queen piece, available to move and can attack*/
